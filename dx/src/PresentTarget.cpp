...
PresentTarget::PresentTarget(ComPtr<ID3D12Device> device,
    ...
    device->CreateRenderTargetView(_backBuffers[0].Get(), nullptr, rtvHandle);

    rtvHandle.Offset(rtvDescriptorSize);
    _swapChain->GetBuffer(1, IID_PPV_ARGS(_backBuffers[1].GetAddressOf()));
    device->CreateRenderTargetView(_backBuffers[1].Get(), nullptr, rtvHandle);
    ...
    _rectScissor = { 0, 0, (LONG)width,(LONG)height };
}
