...
void RayTracingPipelineShader::_gpuToCpuTransfer(GpuToCpuBuffers& resources, UINT64 byteSize) {
    ...
    D3D12_RESOURCE_BARRIER outputBufferResourceBarrier{CD3DX12_RESOURCE_BARRIER::Transition(
        resources.outputBuffer.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COPY_SOURCE)};
...
RayTracingPipelineShader::RayTracingPipelineShader(std::string           shader,
    ...
    _raytracingOutput     = new RenderTexture(
        IOEventDistributor::screenPixelWidth, IOEventDistributor::screenPixelHeight, TextureFormat::RGBA_FLOAT);
    ...
    // This is a root signature that is shared across all raytracing shaders invoked during a DispatchRays() call.
    {
        ...
        _dxrDevice->CreateRootSignature(
            1, blob->GetBufferPointer(), blob->GetBufferSize(), IID_PPV_ARGS(&(_raytracingGlobalRootSignature)));
    }
    ...
    // This is a root signature that enables a shader to have unique arguments that come from shader tables.
    {
        ...
        _dxrDevice->CreateRootSignature(
            1, blob->GetBufferPointer(), blob->GetBufferSize(), IID_PPV_ARGS(&(_raytracingLocalRootSignature)));
    }
    ...
    dxcCompiler->Compile(
        pSource, L"rayTracingUberShader.hlsl", L"", shWide, nullptr, 0, &define, 1, nullptr, &dxcResult);
    ...
    D3D12_SHADER_BYTECODE libdxil =
        CD3DX12_SHADER_BYTECODE((unsigned char*)pProgram->GetBufferPointer(), pProgram->GetBufferSize());
    ...
    // Define explicit shader association for the local root signature.
    {
        auto rootSignatureAssociation =
            raytracingPipeline.CreateSubobject<CD3D12_SUBOBJECT_TO_EXPORTS_ASSOCIATION_SUBOBJECT>();
    ...
    _geometryDescSize = bottomLevelModels.size();
    UINT modelIndex   = 0;
    struct Vertex {
        float pos[3];
        float normal[3];
    ...
    for (auto entity : bottomLevelModels) {
        ...
        auto indexGPUAddress =
            (*entity.second->getFrustumVAO())[0]->getIndexResource()->getResource()->GetGPUVirtualAddress();
        auto vertexGPUAddress =
            (*entity.second->getFrustumVAO())[0]->getVertexResource()->getResource()->GetGPUVirtualAddress();
        ...
        _vertexBuffer[modelIndex].nameId   = entity.second->getModel()->getName();

        // Vertex buffer is passed to the shader along with index buffer as a descriptor table.
        // Vertex buffer descriptor must follow index buffer descriptor in the descriptor heap.
        UINT descriptorIndexIB =
            _createBufferSRV(&_indexBuffer[modelIndex], _geometryDesc[modelIndex].Triangles.IndexCount, 0);
        UINT descriptorIndexVB =
            _createBufferSRV(&_vertexBuffer[modelIndex],
    ...
    for (auto model : bottomLevelInstances) {
        ...
        // resources that will contain acceleration structures must be created in the state
        ...
        //  - the system will be doing this type of access in its implementation of acceleration structure builds behind
        //  the scenes.
        //  - from the app point of view, synchronization of writes/reads to acceleration structures is accomplished
        //  using UAV barriers.
                ...
                for (std::string textureName : textureNames) {
                    if (textureName.find("leaf") != std::string::npos) {
                        ...
                        if (exists == false) {
                            ...
                            break;
                        } else if (texture->getTransparency() == true && exists == true && existingEntity != nullptr) {
    ...
    _hUAVDescriptor =
        new CD3DX12_CPU_DESCRIPTOR_HANDLE(_descriptorHeap->GetCPUDescriptorHandleForHeapStart(), 0, _descriptorSize);
    _uavDesc                    = {};
...
void RayTracingPipelineShader::doRayTracing(Entity* entity, Light* light) {
    ...
    auto inverseProj = (light->getLightMVP().getProjectionMatrix() * light->getLightMVP().getViewMatrix() *
                        light->getLightMVP().getModelMatrix())
                           .inverse();

    auto inverseCameraView = (entity->getMVP()->getViewMatrix() * entity->getMVP()->getModelMatrix()).inverse();

    auto inverseLightView = (light->getLightMVP().getViewMatrix() * light->getLightMVP().getModelMatrix()).inverse();
    ...
    auto gpuDesc = CD3DX12_GPU_DESCRIPTOR_HANDLE(_descriptorHeap->GetGPUDescriptorHandleForHeapStart(),
    ...
    _dxrCommandList->SetComputeRootConstantBufferView(GlobalRootSignatureParams::SceneConstantSlot, cbGpuAddress);
    ...
    _dxrCommandList->ResourceBarrier(
        1, &CD3DX12_RESOURCE_BARRIER::UAV(_raytracingOutput->getResource()->getResource().Get()));
}
...
ComPtr<ID3D12DescriptorHeap> RayTracingPipelineShader::getUVStructuredBuffer() {

    if (_initUvStructuredBuffer == false) {
        ...
        _dxrDevice->CreateDescriptorHeap(&srvHeapDesc, IID_PPV_ARGS(_uvStructuredBufferDescHeap.GetAddressOf()));
        ...
        _uvStructuredBuffer.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
        _uvStructuredBuffer.Format                  = DXGI_FORMAT_UNKNOWN;
        _uvStructuredBuffer.ViewDimension           = D3D12_SRV_DIMENSION_BUFFER;
        _uvStructuredBuffer.Buffer.FirstElement     = 0;

        for (int i = 0; i < _vertexBuffer.size(); i++) {

            if (_vertexBuffer[i].nameId.find("tree") != std::string::npos) {
                _uvStructuredBuffer.Buffer.NumElements         = _geometryDesc[i].Triangles.VertexCount;
                _uvStructuredBuffer.Buffer.StructureByteStride = sizeof(Vertex);
                _uvStructuredBuffer.Buffer.Flags               = D3D12_BUFFER_SRV_FLAG_NONE;
                CD3DX12_CPU_DESCRIPTOR_HANDLE hDescriptor(_uvStructuredBufferDescHeap->GetCPUDescriptorHandleForHeapStart());
                _dxrDevice->CreateShaderResourceView(_vertexBuffer[i].resource.Get(), &_uvStructuredBuffer, hDescriptor);
                break;
            }
        }
        _initUvStructuredBuffer = true;
    }
    return _uvStructuredBufferDescHeap;
}