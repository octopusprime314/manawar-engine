...
void OSP::generateGeometryOSP(std::vector<Entity*>& entities) {
    ...
    Cube* rootCube       = new Cube(_cubicDimension,
                                    _cubicDimension,
                                    _cubicDimension,
...
void OSP::_getChildren(std::vector<Cube>* cubes,
                       OctNode<Cube*>*    node) {
...
void OSP::updateOSP(std::vector<Entity*>& entities) {
    ...
    //Go through all of the models and populate
    for (auto entity : entities){
        //Only do osp updates if the model is active
        if (entity->getStateVector()->getActive()) {
...
bool OSP::_insertSphereSubspaces(Entity*         entity,
                                 Sphere&         sphere,
                                 OctNode<Cube*>* node) {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        if (childrenNodes[i] != nullptr &&
            GeometryMath::sphereCubeDetection(&sphere, childrenNodes[i]->getData())) {
...
void OSP::_buildOctetTree(Cube* cube, OctNode<Cube*>* node) {
    ...
    //Take any dimension and divide by 2
    float cubicDimension = cube->getLength() / 2.0f;
    //New cubic position values
    float dim = cubicDimension / 2.0f;
        ...
        for (auto triangleIndex : triangleMap.second) {
            ...
            for (auto cube : cubes) {
                ...
                if (GeometryMath::triangleCubeDetection(triangleIndex.second, cube)) {
                    nodes[cubeIndex]->addGeometry(triangleMap.first,
        ...
        for (Sphere* sphere : sphereMap.second) {
            ...
            for (auto cube : cubes) {
                ...
                if (GeometryMath::sphereCubeDetection(sphere, cube)) {
                    ...
                    nodes[cubeIndex]->addGeometry(sphereMap.first,
                                                  sphere);
                    cubesEntered[cubeIndex]++;
                }
                cubeIndex++;
    ...
    for (auto cube : cubes) {
        if (cubesEntered[cubeIndex] > _maxGeometries) {
            //Recursive call to dig deeper into octary space partition tree
            _buildOctetTree(cube, nodes[cubeIndex]);
        }
        //If a subspace has less than _maxGeometries primitive count,
        //then add it to the leaves list for collision detection
        else {
