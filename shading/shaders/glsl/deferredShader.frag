...
void main() {
    ...
    if (views == 0) {
        ...
        if (normal.x == 0.0 && normal.y == 0.0 && normal.z == 0.0) {
            vec4 dayColor   = texture(skyboxDayTexture,
                                    vec3(vsData.vsViewDirection.x, vsData.vsViewDirection.y, vsData.vsViewDirection.z));
            vec4 nightColor = texture(
                skyboxNightTexture, vec3(vsData.vsViewDirection.x, vsData.vsViewDirection.y, vsData.vsViewDirection.z));
            fragColor =
            ...
            //        pointLighting += (dot(pointLightDirNorm, normalizedNormal)) * (1.0 -
            ...
            //
            //        vec3 cubeMapTexCoords = (viewToModelMatrix * vec4(position.xyz,1.0)).xyz - (viewToModelMatrix *
            //        vec4(pointLightPositions[i].xyz, 1.0)).xyz; float distance = length(cubeMapTexCoords); float
            //        cubeDepth = texture(depthMap, normalize(cubeMapTexCoords.xyz)).x*pointLightRanges[i];
            //
    ...
    //
    //  vec3 cubeMapTexCoords = (viewToModelMatrix * vec4(position.xyz,1.0)).xyz - (viewToModelMatrix *
    //  vec4(pointLightPositions[0].xyz, 1.0)).xyz; float cubeDepth = texture(depthMap,
    //  normalize(cubeMapTexCoords.xyz)).x; fragColor = vec4(vec3(cubeDepth), 1.0); gl_FragDepth = 0.1;
    //}
    else if (views == 9) {
