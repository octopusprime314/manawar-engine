...
// uniform sampler2D   mapDepthTexture;          //depth texture data array with values 1.0 to 0.0, with 0.0 being
// closer uniform samplerCube depthMap;                 //cube depth map for point light shadows
...
void main() {
    ...
    if (diffuse.a < 0.1) {
        ...
        if (normalizedLight.y <= 0.0) {
            ...
            // if(shadowTextureCoordinates.x <= 1.0 && shadowTextureCoordinates.x >= 0.0 && shadowTextureCoordinates.y
            // <= 1.0 && shadowTextureCoordinates.y >= 0.0){
            ...
            //  vec2 shadowTextureCoordinatesMap = shadowMappingMap.xy * vec2(0.5,0.5) + vec2(0.5,0.5);
            //
            //  if ( texture( mapDepthTexture, shadowTextureCoordinatesMap).x < (shadowMappingMap.z * 0.5 + 0.5) -
            //  bias){
            //      directionalShadow = shadowEffect;
            //  }
            //}
        ...
        //      pointLighting += (dot(pointLightDirNorm, vsData.normalOut)) * (1.0 -
        //      (distanceFromLight/(pointLightRanges[i]))) * pointLightColors[i]; totalPointLightEffect +=
        //      dot(pointLightDirNorm, vsData.normalOut) * (1.0 - (distanceFromLight/(pointLightRanges[i])));
        //
        //      vec3 cubeMapTexCoords = (viewToModelMatrix * vec4(vsData.positionOut.xyz,1.0)).xyz - (viewToModelMatrix
        //      * vec4(pointLightPositions[i].xyz, 1.0)).xyz; float distance = length(cubeMapTexCoords); float cubeDepth
        //      = texture(depthMap, normalize(cubeMapTexCoords.xyz)).x*pointLightRanges[i]; float bias = 0.05;
        ...
        vec3 lightComponentIllumination =
