...
void main() {
    ...
    vsData.positionOut =
        view * model *
        vec4(vertexIn.xyz, 1.0); // store only in model space so deferred shadow rendering is done properly

    vsData.normalOut =
        vec3((normalMatrix * vec4(normalIn, 0.0)).xyz); // Transform normal coordinate in with the normal matrix
