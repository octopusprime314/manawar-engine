...
AssetTexture::AssetTexture(std::string textureName, bool cubeMap) : Texture(textureName), _alphaValues(false) {
...
AssetTexture::AssetTexture(void*                              data,
                           UINT                               width,
                           UINT                               height,
                           ComPtr<ID3D12GraphicsCommandList>& cmdList,
                           ComPtr<ID3D12Device>&              device)
...
void AssetTexture::_build2DTextureDX(void*                              data,
                                     UINT                               width,
                                     UINT                               height,
                                     ComPtr<ID3D12GraphicsCommandList>& cmdList,
                                     ComPtr<ID3D12Device>&              device) {
    ...
    device->CreateDescriptorHeap(&descHeapSampler, IID_PPV_ARGS(_samplerDescriptorHeap.GetAddressOf()));
    ...
    device->CreateSampler(&samplerDesc, _samplerDescriptorHeap->GetCPUDescriptorHandleForHeapStart());
}
...
void AssetTexture::_buildCubeMapTextureGL(std::string skyboxName) {
    ...
    for (unsigned int i = 0; i < 6; ++i) {
        ...
        _decodeTexture(textureName, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, GL_TEXTURE_CUBE_MAP);
...
void AssetTexture::_buildCubeMapTextureDX(std::string                        skyboxName,
    ...
    _textureBuffer = new ResourceBuffer(
        cubeMapData.data(), 6, static_cast<UINT>(cubeMapData.size()), _width, _height, rowPitch, cmdList, device);
...
void AssetTexture::_decodeTexture(std::string textureName, uint32_t textureType, uint32_t target) {
    ...
    if (errorCode != GL_NO_ERROR) {
        char msg[1024];
        sprintf(msg, "%s:%d %s() error code is: %d.\n", __FILE__, __LINE__, __func__, errorCode);
