...
CubeMap::CubeMap(unsigned int width, unsigned int height, bool isDepth) : Texture("cube", width, height) {
    ...
    for (unsigned int i = 0; i < 6; ++i) {
        ...
        if (isDepth) {
            ...
            glTexImage2D(
                GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA, _width, _height, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
        }
    }
...
unsigned int CubeMap::getCubeFrameBufferContext() { return _cubeFrameBuffer; }

unsigned int CubeMap::getCubeMapContext() { return _cubemap; }
