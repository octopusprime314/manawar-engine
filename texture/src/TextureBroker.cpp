    ...
    if (_textures.find(textureName) == _textures.end()) {
        if (EngineManager::getGraphicsLayer() == GraphicsLayer::OPENGL) {
            ...
            _textures[textureName] =
                new AssetTexture(textureName, DXLayer::instance()->getCmdList(), DXLayer::instance()->getDevice());
        }
    }
}
...
void TextureBroker::addLayeredTexture(std::vector<std::string> textureNames) {
    ...
    if (_layeredTextures.find(sumString) == _layeredTextures.end()) {
        ...
        if (EngineManager::getGraphicsLayer() == GraphicsLayer::OPENGL) {
            ...
            _layeredTextures[sumString] =
                new LayeredTexture(textures, DXLayer::instance()->getCmdList(), DXLayer::instance()->getDevice());
        }
    }
}
...
void TextureBroker::addCubeTexture(std::string textureName) {
    if (_textures.find(textureName) == _textures.end()) {
        if (EngineManager::getGraphicsLayer() == GraphicsLayer::OPENGL) {
            ...
            _textures[textureName] = new AssetTexture(
                textureName, DXLayer::instance()->getCmdList(), DXLayer::instance()->getDevice(), true);
        }
    }
}
        ...
        for (auto& assetTexture : assetTextures) {
            ...
            if (assetTexture->getName().compare(textureName) == 0) {
                ...
                if (EngineManager::getGraphicsLayer() == GraphicsLayer::OPENGL) {
                    ...
                    layeredTexture.second->setTexture(new AssetTexture(
                        textureName, DXLayer::instance()->getCmdList(), DXLayer::instance()->getDevice()));
                }
            }
        }
    }
