...
struct CameraVector {
    ...
    CameraVector(const Vector4& d, const Vector4& r, float t) : direction(d), rotation(r), time(t) {}
};
...
class VectorCamera : public Camera {
  public:
    ...
    void         setVectors(const std::vector<CameraVector>& vectors);
  ...
  private:
    ...
    float                     _elapsedTime;
    Matrix                    _inversion;
    std::vector<CameraVector> _vectors;
};